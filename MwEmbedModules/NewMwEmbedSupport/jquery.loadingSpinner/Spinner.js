(function(window, document, undefined) {
	...
	/**
	 * Copyright (c) 2011 Felix Gnass [fgnass at neteye dot de] Licensed under
	 * the MIT license
	 */
	...
	/**
	 * Inserts child1 before child2. If child2 is not specified, child1 is
	 * appended. If child2 has no parentNode, child2 is appended first.
	 */
	...
	function addAnimation(alpha, trail, i, lines) {
		var name = [ 'opacity', trail, ~~(alpha * 100), i, lines ].join('-'), start = 0.01
		+ i / lines * 100, z = Math.max(1 - (1 - alpha) / trail
				* (100 - start), alpha), prefix = useCssAnimations.substring(0,
		...
		this.opts = defaults(o || {}, {
			...
			spin : function(target) {
				...
				if (!useCssAnimations) {
					...
					var o = self.opts, i = 0, f = 20 / o.speed, ostep = (1 - o.opacity)
					/ (f * o.trail / 100), astep = f / o.lines;
	...
	proto.lines = function(el, o) {
		...
		function fill(color, shadow) {
			return css(createEl(), {
				...
				transform : 'rotate(' + ~~(360 / o.lines * i)
		...
		for (; i < o.lines; i++) {
			seg = css(createEl(), {
				...
				&& addAnimation(o.opacity, o.trail, i, o.lines) + ' '
			...
			proto.lines = function(el, o) {
				...
				function seg(i, dx, filter) {
					ins(g, ins(css(grp(), {
						...
						opacity : 0
					}) // transparent stroke to fix color bleeding upon opacity
					// change
					)));
				}
